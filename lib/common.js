 ...
 * Support for urls in http.request/http.get was added in v10.9.0
...
const requestParameters = function(input, options, callback) {
  if (semver.satisfies(process.version, '<10.9.0')) {
    return { options: input, callback: options }
  }

  if (typeof input === 'string' && typeof options === 'object') {
    options = Object.assign({}, urlToOptions(new URL(input)), options)
  } else if (typeof input === 'string' && typeof options !== 'object') {
    callback = options
    options = Object.assign({}, urlToOptions(new URL(input)))
  } else {
    callback = options
    options = input
  }
